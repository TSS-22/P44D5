...
def resolve_references(data):
    for value in data:
        if isinstance(value, list):
            data[value] = [data[ref.replace("$ref:", "")] for ref in value if isinstance(ref, str) and ref.startswith("$ref:")]
    return data
...
def init_state_app():
    ...
    return ({
        ...
        "knob_quadrant_playModes": (data_settings["pot_max_value"]/len(data_options_play["knob_values_playModes"])),
        "knob_quadrant_PlayType": (data_settings["pot_max_value"]/len(data_options_play["knob_values_playTypes"])),
    })
...
def init_options_play():
    ...
    for key in data_options_play["playModes_chordProg"]:
        ...
        for val in data_options_play["playModes_chordProg"][key]:
            mode_prog_chord[key].append(data_options_play[data_options_play["ionian_chord_prog"][val]])
